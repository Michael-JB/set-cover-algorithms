...
std::vector<uint64_t> parseLine(char* line, const char delimiter) {
  std::vector<uint64_t> data;
  while (*line) {
    const uint64_t val = strtol(line, &line, 10);
    data.push_back(val);
...
int main(int argc, char** argv) {
  ...
  if (setFile.is_open()) {
    ...
    while (getline(setFile, line)) {
      const std::vector<uint64_t> data = parseLine(const_cast<char*>(line.c_str()), ' ');
      dfg.offerSet(data, index++);
    }
    const auto t2 = std::chrono::high_resolution_clock::now();
    const std::vector<uint64_t> cover = dfg.extractCover();
    ...
    std::cout << "File: " << setFileName << std::endl
              ...
              << "Bytes read/written from disk: " << (dfg.getBytesWritten() + dfg.getBytesRead()) * 0.000001 << "MB" << std::endl
