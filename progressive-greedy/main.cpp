...
std::unordered_set<int> coveredElements, cover, coveredElementsBackup;
std::unordered_map<int,int> backupCertificate;
...
// Processes a set, folding if appropriate
void offerSet(const std::vector<int>& set, const int index, const float threshold, const bool fold) {
  ...
  if (fold) {
    // Add all sets with a non-zero contribution to a backup cover certificate, ready for folding
    const std::vector<int> diffFold = setDifference(set, coveredElementsBackup);
    for (int element : diffFold) {
      std::unordered_map<int,int>::iterator elem = backupCertificate.find(element);
      if (elem == backupCertificate.end()) {
        backupCertificate.insert({element, index});
      } else {
        elem->second = index;
      }
    }
    coveredElementsBackup.insert(diffFold.begin(), diffFold.end());
  }
}
...
int main(int argc, char** argv) {
  ...
  if (setFile.is_open()) {
    ...
    for (int j = 1; j <= p; j++) {
      ...
      const float threshold = calculateThreshold(delta, p + 1, j);
      int index = 0;
      while (getline(setFile, line)) {
        const std::vector<int> data = parseLine(const_cast<char*>(line.c_str()), ' ');
        offerSet(data, index++, threshold, j == p);
      }
    }

    // Post processing to combine cover certificates
    for (const std::pair<const int,const int>& element : backupCertificate) {
      std::unordered_set<int>::const_iterator elementInCover = coveredElements.find(element.first);
      if (elementInCover == coveredElements.end()) {
        cover.insert(element.second);
      }
    }
    ...
    std::cout << "File: " << setFileName << std::endl
              << "Algorithm: Progressive Greedy (p=" << p << ")" << std::endl
